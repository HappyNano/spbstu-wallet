...
TEST_P(DatabaseTest, CreateAndDropTable) {
    ASSERT_TRUE(db_->makeTransaction()->createTable("test_table", getTestTableColumns()));

    auto result = db_->makeTransaction()->executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='test_table'");
    ASSERT_TRUE(result.has_value());
    ASSERT_FALSE(result->empty());

    ASSERT_TRUE(db_->makeTransaction()->dropTable("test_table"));

    result = db_->makeTransaction()->executeQuery("SELECT name FROM sqlite_master WHERE type='table' AND name='test_table'");
    ASSERT_TRUE(result.has_value());
...
TEST_P(DatabaseTest, InsertData) {
    ...
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age", "salary", "active" }, { "1", "Alice", "30", "50000.50", "1" }));

    auto result = db_->makeTransaction()->select("test_table", { "id", "name", "age", "salary", "active" });
    ASSERT_TRUE(result.has_value());
...
TEST_P(DatabaseTest, SelectData) {
    ...
    // Multiple insert
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age", "salary", "active" }, { "1", "Alice", "30", "50000.50", "1" }));
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age", "salary", "active" }, { "2", "Bob", "25", "45000.75", "0" }));
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age", "salary", "active" }, { "3", "Charlie", "35", "55000.25", "1" }));

    // Check data with column list
    auto result = db_->makeTransaction()->select("test_table", { "id", "name", "age" });
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 3);

    // Check data with empty columns list
    result = db_->makeTransaction()->select("test_table", {});
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 3);
...
TEST_P(DatabaseTest, UpdateData) {
    ...
    ASSERT_TRUE(db_->makeTransaction()->update("test_table", updates, "id = 1"));

    // Check data updated
    auto result = db_->makeTransaction()->select("test_table", { "name", "age", "active" });
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 1);
...
TEST_P(DatabaseTest, DeleteData) {
    ...
    // Multiple instert
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age" }, { "1", "Alice", "30" }));
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age" }, { "2", "Bob", "25" }));
    ASSERT_TRUE(db_->makeTransaction()->insert("test_table", { "id", "name", "age" }, { "3", "Charlie", "35" }));

    auto result = db_->makeTransaction()->select("test_table", { "id" });
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 3);

    // Delete one
    ASSERT_TRUE(db_->makeTransaction()->deleteFrom("test_table", "id = 2"));
    // Check deletion
    result = db_->makeTransaction()->select("test_table", { "id" });
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 2);

    // Delete all
    ASSERT_TRUE(db_->makeTransaction()->deleteFrom("test_table", ""));
...
TEST_P(DatabaseTest, DataTypes) {
    ...
    ASSERT_TRUE(db_->makeTransaction()->createTable("types_test", cols));

    // Check insert multiple types
    ASSERT_TRUE(db_->makeTransaction()->insert(
     "types_test",
     { "int_val", "real_val", "text_val", "bool_val", "date_val", "timestamp_val" },
     { "42", "3.14", "hello", "1", "2023-07-15", "2023-07-15 14:30:00" }));

    // Check data
    auto result = db_->makeTransaction()->select("types_test", {});
    ASSERT_TRUE(result.has_value());
    ASSERT_EQ(result->size(), 1);
...
TEST_P(DatabaseTest, Constraints) {
    ...
    ASSERT_TRUE(db_->makeTransaction()->createTable("pk_test", pkCols));
    ASSERT_TRUE(db_->makeTransaction()->insert("pk_test", { "id", "name" }, { "1", "Test" }));

    // Try insert dublicate key
    ASSERT_FALSE(db_->makeTransaction()->insert("pk_test", { "id", "name" }, { "1", "Test Duplicate" }));
    ...
    ASSERT_TRUE(db_->makeTransaction()->createTable("nn_test", nnCols));
    // Try insert without NOT NULL
    ASSERT_FALSE(db_->makeTransaction()->insert("nn_test", { "id" }, { "1" }));
    ...
    ASSERT_TRUE(db_->makeTransaction()->createTable("uq_test", uqCols));
    ASSERT_TRUE(db_->makeTransaction()->insert("uq_test", { "id", "code" }, { "1", "ABC" }));
    ASSERT_FALSE(db_->makeTransaction()->insert("uq_test", { "id", "code" }, { "2", "ABC" }));
}

TEST_P(DatabaseTest, ErrorHandling) {
    // Select non existent table
    auto result = db_->makeTransaction()->select("non_existent_table", {});
    ASSERT_FALSE(result.has_value());

    // Drop non existent table
    ASSERT_FALSE(db_->makeTransaction()->dropTable("non_existent_table"));
    ...
    ASSERT_TRUE(db_->makeTransaction()->createTable("error_test", cols));

    // Update with wrong condition
    std::vector< std::pair< std::string, std::string > > updates = {
        { "name", "Test" }
    };
    ASSERT_FALSE(db_->makeTransaction()->update("error_test", updates, "non_existent_column = 1"));

    // Delete with wrong condition
    ASSERT_FALSE(db_->makeTransaction()->deleteFrom("error_test", "non_existent_column = 1"));
}

TEST_P(DatabaseTest, CheckExistence) {
    ASSERT_TRUE(db_->makeTransaction()->createTable("test_table", getTestTableColumns()));

    EXPECT_TRUE(db_->makeTransaction()->isTableExist("test_table"));
    EXPECT_FALSE(db_->makeTransaction()->isTableExist("non_existent_table"));
}
...
TEST_P(SQLiteDatabaseTest, ConnectInMemory) {
    ...
    ASSERT_TRUE(db->connectInMemory());
    ASSERT_TRUE(db->isReady());
}
